...
const Dashboard: React.FC<DashboardProps> = ({ onLogout, settings, onUpdateSettings }) => {
    ...
    const generateTandaTerimaPDF = async () => {
        ...
        if (settings.logo) {
            try {
                // Add logo to header (approx 20mm x 20mm)
        ...
        const bawasluSplit = doc.splitTextToSize(bawasluName, pageWidth - 80); // More margin if logo present
        ...
        y = Math.max(y, 40); // Ensure header spacing
